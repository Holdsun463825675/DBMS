...
import ConfirmModal from "../../Common/ConfirmModal"; // 引入专用的 UserHome 样式文件
...
export default function UserHome(props) {
    ...
    return (
        <Container fluid className="user-home-container">
            ...
            <h2 className="user-welcome-text">欢迎回来，普通用户{userName}，您想要看点什么呢？</h2>
            <Row className="d-flex justify-content-center user-option-row">
                {options.map(({title, href}, idx) => (
                    <Col key={idx} xs={10} sm={6} md={4} className="mb-4">
                        <Card className="user-option-card text-center"
                        ...
                        >
                            ...
                            />
                            <Card.Body>
                                <Card.Text className="user-card-text">{title}</Card.Text>
                            </Card.Body>
                        </Card>
                    </Col>
                ))}
